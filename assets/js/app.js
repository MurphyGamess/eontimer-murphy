...
const playBeep = () => {
  ...
  // SUTIL MEJORA: Usar currentTime de AudioContext para mejor sincronización
  const source = audioContext.createBufferSource();
  source.buffer = beepBuffer;
  source.connect(audioContext.destination);
  source.start(audioContext.currentTime); // Usar audio clock en lugar de start() sin parámetro
...
const startTimer = (millis, onComplete) => {
  ...
  timerInterval = setInterval(() => {
    ...
    // MEJORADO: Usar AudioContext para beeps precisos pero mantener lógica simple
...
// 🎮 INICIALIZAR SISTEMA DE POKEMON FLOTANTES - ELECTRON COMPATIBLE
window.addEventListener('DOMContentLoaded', () => {
  console.log('🎮 Initializing Pokemon Floaters System for Electron...');
  
  // Esperar a que todos los scripts se carguen en Electron
  setTimeout(() => {
    // Inicializar sistema de Pokemon flotantes
    if (typeof PokemonFloaters !== 'undefined') {
      try {
        // window.pokemonFloaters = new PokemonFloaters(); // TEMPORALMENTE DESACTIVADO PARA USAR NUEVO SISTEMA
        console.log('⚠️ Pokemon Floaters DESACTIVADO - Usando nuevo sistema del index.html');
        
        // Exponer métodos de pokeball para testing
        window.testPokeballEffect = (ballType, pokemonName = 'pikachu', isShiny = false) => {
          const position = { x: window.innerWidth / 2, y: window.innerHeight / 2 };
          window.pokemonFloaters.createSimeydotmePokeballSequence(ballType, position, () => {
            console.log(`✅ ${ballType} test completed`);
          }, pokemonName, isShiny);
        };
        
        // Auto-iniciar el sistema de pokémon aleatorios
        setTimeout(() => {
          console.log('🔄 Starting Pokemon random generation...');
          if (window.pokemonFloaters && typeof window.pokemonFloaters.startNewGeneration === 'function') {
            // Forzar inicio de nueva generación
            window.pokemonFloaters.startNewGeneration();
            console.log('✅ Pokemon generation FORCED START');
          }
        }, 3000); // Más tiempo para Electron
        
      } catch (error) {
        console.error('❌ Error initializing Pokemon Floaters:', error);
      }
    } else {
      console.warn('⚠️ PokemonFloaters class not found - retrying...');
      // Retry después de más tiempo
      setTimeout(() => {
        if (typeof PokemonFloaters !== 'undefined') {
          // window.pokemonFloaters = new PokemonFloaters(); // TEMPORALMENTE DESACTIVADO PARA USAR NUEVO SISTEMA
          console.log('⚠️ Pokemon Floaters retry DESACTIVADO - Usando nuevo sistema del index.html');
        }
      }, 2000);
    }
  }, 1000); // Delay inicial para Electron
  
  // Inicializar sistema de pokeball effects avanzados
  if (typeof AdvancedPokeballEffects !== 'undefined') {
    window.advancedPokeballEffects = new AdvancedPokeballEffects();
    console.log('✅ Advanced Pokeball Effects initialized');
  } else {
    console.warn('⚠️ AdvancedPokeballEffects class not found');
  }
});

// 🎯 FUNCIONES GLOBALES PARA DEBUGGING - REDIRIGIDAS AL NUEVO SISTEMA
window.debugPokemon = () => {
  if (window.debugPokemonSystem) {
    window.debugPokemonSystem();
  } else {
    console.log('⚠️ Sistema anterior desactivado - usar debugPokemonSystem() del nuevo sistema');
  }
};

window.forceSpawnPokemon = (pokemonName, isShiny = false) => {
  if (window.forceSpawnPokemon) {
    window.forceSpawnPokemon();
    console.log(`🎮 Usando nuevo sistema de spawn`);
  } else {
    console.log('⚠️ Sistema anterior desactivado - usar forceSpawnPokemon() del nuevo sistema');
  }
};

window.clearAllPokemon = () => {
  if (window.clearAllPokemon) {
    window.clearAllPokemon();
  } else {
    console.log('⚠️ Sistema anterior desactivado - usar clearAllPokemon() del nuevo sistema');
  }
};
